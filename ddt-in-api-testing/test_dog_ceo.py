...
def test_request_random_quantity_of_dogs(quantity_dogs):
    response = requests.get(f'{URL}breeds/image/random/{quantity_dogs}')
    schema = {
        'message': {'type': 'list'},
        'status': {'type': 'string'}
    }
...
def test_receive_all_dog_images():
    response = requests.get(f'{URL}breed/hound/images')
    schema = {
        'message': {'type': 'list'},
        'status': {'type': 'string'}
    }
...
def test_receive_dogs_of_each_sub_breed(sub_breed):
    response = requests.get(f'{URL}breed/hound/{sub_breed}/images')
    schema = {
        'message': {'type': 'list'},
        'status': {'type': 'string'}
    }
...
def test_multiple_images_from_sub_breed_collection(sub_breed, expected_code, quantity_dogs):
    response = requests.get(f'{URL}breed/hound/{sub_breed}/images/random/{quantity_dogs}')
    schema = {
        'code': {'type': 'integer'},
